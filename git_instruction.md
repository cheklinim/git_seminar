...
Например:
Мы сделали fork проекта, хотим помочь человеку сделать его проект лучше. Когда мы готовы спредложить ему свои доработки, мы должны предложить их автору. Для этого существует:

    pull request

В результате наш вариант кода отправится автору репозитория, который мы до этого forkнули. Он сможет принять или отклонить предложенные нами изменения.
